...
func ScalarBaseMult(k *big.Int) *ecdsa.PublicKey {
	...
	key.Curve = secp256k1.Secp256k1()
	key.X, key.Y = secp256k1.Secp256k1().ScalarBaseMult(k.Bytes())
	return key
}
...
func ScalarMult(k *big.Int, B *ecdsa.PublicKey) *ecdsa.PublicKey {
	key := new(ecdsa.PublicKey)
	key.Curve = secp256k1.Secp256k1()
	key.X, key.Y = secp256k1.Secp256k1().ScalarMult(B.X, B.Y, k.Bytes())
	return key
}
...
func Add(a, b *ecdsa.PublicKey) *ecdsa.PublicKey {
	key := new(ecdsa.PublicKey)
	key.Curve = secp256k1.Secp256k1()
	key.X, key.Y = secp256k1.Secp256k1().Add(a.X, a.Y, b.X, b.Y)
	return key
}
